...
class JobinjaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for job in job_links:
            ...
            if link is not None:
                ...
                yield scrapy.Request(link, callback=self.parse_job_details, meta={'title': title})
    ...
    def parse_job_details(self, response):
        ...
        job_description_parts = response.css('.o-box__text::text, .o-box__text.s-jobDesc.c-pr40p::text').getall()
        job_description = ' '.join([part.strip() for part in job_description_parts if part.strip()]) if job_description_parts else 'N/A'
        ...
        employment_type = location = gender = minimum_work_experience = salary = skills = job_classification = None
